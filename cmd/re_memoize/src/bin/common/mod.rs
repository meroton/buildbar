...
//! `main`). Not part of the public `re_memoize` library: it's only
//! meaningful to re-memoize's own checks, not to consumers of the crate.

use std::fs;
use std::path::{Path, PathBuf};

use re_memoize::error::{Error, IoResultExt};
...
impl ScratchDir {
    ...
    /// so `ls $TMPDIR/re-memoize` stays legible with several checks' dirs
    ...
    pub fn new(label: &str, keep: bool) -> Result<Self, Error> {
        // All of this project's temp dirs live under one re-memoize/
        // parent, so they're easy to find (`ls $TMPDIR/re-memoize`)
        // instead of scattered directly in the OS temp dir among
        // everyone else's.
        let base = std::env::temp_dir().join("re-memoize");
