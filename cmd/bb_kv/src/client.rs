...
impl RemoteClient {
    pub async fn connect(remote: &str, instance_name: String) -> Result<Self, Error> {
        let connect = || Endpoint::from_shared(remote.to_owned());
        let endpoint = connect().map_err(|source| Error::Connect {
